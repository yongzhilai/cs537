...
#define NUM_PARTITIONS NUM_REDUCERS
#endif
...
int main(int argc, char *argv[]) {
    ...
    for (int iter = 0; iter < NUM_ITERATION; iter++) {
        ...
        for (int i = 0; i < NUM_PARTITIONS; i++) {
            sprintf(buf, "wordcount_multi%d_%s(%d).out", iter, FILE_OUTPUT_SUFFIX, i);
        ...
        printf("** %.3f ms elapsed for %dth iteration **\n", cpu_time_used, iter);
